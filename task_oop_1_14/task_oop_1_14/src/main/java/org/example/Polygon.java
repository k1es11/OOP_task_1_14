...
public class Polygon extends GeometricObject {
    ...
    public static void sortClockwise(List<Point> polygon) {
        ...
        List<Point> sortedPoints = new ArrayList<>(polygon);
        sortedPoints.sort(new ClockwiseComparator(centroid));
    }
    ...
    public static List<Point> removeInteriorVertices(List<Point> polygon, List<Point> intersections) {
        ...
        for (Point p : polygon) {
            if (isInsidePolygon(intersections, p)) {
    ...
    public GeometricObject subtract(GeometricObject obj) {
        if (obj instanceof Polygon other) {
            ...
            List<Point> subtractedPolygon = new ArrayList<>(this.vertices);

            sortClockwise(subtractedPolygon);
            for (Point p : other.vertices) {
                if (isInsidePolygon(subtractedPolygon, p)) {
                    subtractedPolygon.add(new Point(p.getX(), p.getY()));
                }
            }

            sortClockwise(subtractedPolygon);
            for (Point intersection : intersections) {
                if (isInsidePolygon(subtractedPolygon, intersection)) {
                    subtractedPolygon.add(new Point(intersection.getX(), intersection.getY()));
                }
            }

            sortClockwise(subtractedPolygon);
            for (Point p : intersections){
                if (subtractedPolygon.contains(p)) subtractedPolygon.remove(p);
                else subtractedPolygon.add(p);
            }
            return new Polygon(subtractedPolygon);
        } else {
            return this;
        }
    }
    ...
    public GeometricObject merge(GeometricObject obj) {
        if (obj instanceof Polygon other) {
            ...
            List<Point> union = new ArrayList<>(this.vertices);
            for (Point p : other.vertices) {
                if (!isInsidePolygon(union, p)) {
            ...
            sortClockwise(union);
            for (Point intersection : intersections) {
                if (!isInsidePolygon(union, intersection)) {
                    union.add(new Point(intersection.getX(), intersection.getY()));
                }
            }

            sortClockwise(union);
            List<Point> cleanedUnion = removeInteriorVertices(union, intersections);
            cleanedUnion.remove(cleanedUnion.size()-1);
            return new Polygon(cleanedUnion);
        } else {
            return this;
        }
    }
